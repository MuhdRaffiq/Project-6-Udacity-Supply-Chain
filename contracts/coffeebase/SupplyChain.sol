...
//  Import the accesscontrol of each role
...
contract SupplyChain is ConsumerRole, DistributorRole, FarmerRole, RetailerRole, Ownable {
  ...
  function buyItem(uint _upc) public payable onlyDistributor verifyCaller(msg.sender)
    // Call modifier to check if upc has passed previous supply chain stage
    forSale(_upc)
  ...
  function receiveItem(uint _upc) public onlyRetailer verifyCaller(msg.sender)
    // Call modifier to check if upc has passed previous supply chain stage
    shipped(_upc)
